...
export function TasksHeader({ searchQuery, setSearchQuery, selectedFilter, setSelectedFilter }: TasksHeaderProps) {
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Daftar aktivasi pasang baru pelanggan yang telah ditugaskan ke tim teknisi.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 bg-secondary/50 p-1 rounded-lg border border-border">
            {["all", "ASSIGNED", "COMPLETED"].map((filter) => (
              <button
                key={filter}
              ...
              >
                {filter === "all" ? "Semua Tugas" : filter === "ASSIGNED" ? "Proses Instalasi" : "Selesai"}
              </button>
            ))}
          </div>
        </div>
